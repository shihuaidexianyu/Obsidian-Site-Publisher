...
- Obsidian 插件包：用于安装到 vault 的插件目录
- 独立 CLI 包：用于在本机执行构建、预览和发布

最终用户建议直接从发行包安装；开发者可以从源码构建。
...
- `obsidian-site-publisher-plugin-<version>.zip`
- `publisher-cli-portable-<version>.zip`

安装步骤：

1. 解压插件包，将其中的 `main.js`、`manifest.json`、`versions.json` 复制到：

```text
<你的Vault>/.obsidian/plugins/obsidian-site-publisher/
```

2. 解压 `publisher-cli-portable-<version>.zip` 到任意目录，例如：

```text
C:\Tools\publisher-cli
```
...
3. 在 Obsidian 中启用 `站点发布`
4. 在插件设置中填写 `CLI 可执行文件路径`
...
- CLI 包当前仍依赖本机已安装的 Node.js 20+
- 插件目录中不会包含 CLI 或 Quartz 运行时
